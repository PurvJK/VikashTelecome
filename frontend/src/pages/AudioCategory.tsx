...
const getMaxPrice = (items: { price: number }[]) => {
...
const AudioCategory = () => {
  const [items, setItems] = useState([...audioProducts]);
  const maxPrice = useMemo(() => getMaxPrice(items), [items]);
  ...
  useEffect(() => {
    const audioCategories = new Set(["speakers", "earbuds", "headphones", "neckbands", "soundbars"]);
    api.fetchProducts({ limit: 300 })
      .then((data) => {
        const filtered = data.items.filter((product) => audioCategories.has(product.category));
